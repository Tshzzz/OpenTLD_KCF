...
void TLD::selectObject(const Mat &img, const Mat &rgb_img,Rect *bb)
{
    //Delete old object
    detectorCascade->release();
    ...
    //cout << bb->x <<" "<< bb->y << " " << bb->width << " "<<bb->height <<endl;
...
{
    Rect *trackerBB = kcf_tracker->trackerBB;
    ...
    if(trackerBB != NULL)
    {
        ...
        {
            kcf_tracker->update_pos(currImg,detectorBB);
    ...
    else if(numClusters == 1)
    {
        ...
        //kcf_tracker->track(prevImg,currImg, prevBB);
        kcf_tracker->update_pos(currImg,detectorBB);
        currBB = tldCopyRect(detectorBB);
        currConf = confDetector;
    }
